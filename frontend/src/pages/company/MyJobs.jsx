...
export default function MyJobs() {
  ...
  const vagas = [
    { id: 1, titulo: "Dev React Senior", status: "Ativa", candidatos: 12, views: 89 },
    { id: 2, titulo: "Designer UI/UX", status: "Ativa", candidatos: 8, views: 42 },
    { id: 3, titulo: "Node.js Backend", status: "Pausada", candidatos: 0, views: 15 },
  ];
        ...
        {vagas.map(vaga => (
          <div key={vaga.id} className="bg-white border border-slate-100 p-5 rounded-3xl shadow-sm">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h3 className="font-bold text-sm text-[#1a233d]">{vaga.titulo}</h3>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${vaga.status === 'Ativa' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
                  ● {vaga.status}
                </span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              ...
              >
                ...
                <span className="text-sm font-black">{vaga.candidatos}</span>
              ...
              <div className="bg-slate-50 p-3 rounded-2xl flex flex-col items-center">
                ...
                <span className="text-sm font-black">{vaga.views}</span>
