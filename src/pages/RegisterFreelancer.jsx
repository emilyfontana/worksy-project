...
export default function RegisterFreelancer() {
  ...
  const [role, setRole] = useState("freelancer"); // 'freelancer' ou 'empresa'
  ...
  return (
    <div className="min-h-screen bg-[#1a233d] flex flex-col justify-between font-sans">
      ...
      <div className="px-6 pt-12 pb-6 flex items-center relative text-white">
        <Link to="/login" className="absolute left-6 p-2 bg-white/10 rounded-full text-white">
          <ChevronLeft size={18} />
        </Link>
      ...
      {/* Formulário no Container Branco */}
      <form onSubmit={handleRegisterSubmit} className="bg-white rounded-t-[2.5rem] flex-1 px-6 pt-8 pb-6 flex flex-col justify-between">
        <div className="space-y-5">
          ...
          {/* Seletores de Tipo de Perfil */}
          <div className="grid grid-cols-2 gap-4">
            {/* Botão Freelancer */}
            <div 
              onClick={() => setRole("freelancer")}
              className={`p-4 rounded-2xl border-2 text-center flex flex-col items-center gap-2 cursor-pointer transition relative ${
                role === "freelancer" ? "border-[#2ecc71] bg-emerald-50/10" : "border-slate-100 bg-white"
              }`}
            >
              ...
              {role === "freelancer" && (
                <div className="absolute -bottom-1.5 bg-[#2ecc71] text-white p-0.5 rounded-full"><Check size={10} strokeWidth={4}/></div>
              )}
            </div>
            ...
            <div 
              ...
              className={`p-4 rounded-2xl border-2 text-center flex flex-col items-center gap-2 cursor-pointer transition relative ${
                role === "empresa" ? "border-[#1a233d] bg-slate-50" : "border-slate-100 bg-white"
              }`}
            >
              ...
              {role === "empresa" && (
                <div className="absolute -bottom-1.5 bg-[#1a233d] text-white p-0.5 rounded-full"><Check size={10} strokeWidth={4}/></div>
              )}
            </div>
          </div>

          {/* Inputs Condicionais baseados na Role */}
          <div>
            ...
            <input 
              type="text" 
              placeholder={role === "freelancer" ? "Rafael Silva" : "TechNova Ltda"} 
              required
          ...
          <div>
            ...
            <input type="email" placeholder="seu@email.com" required className="w-full bg-white border border-slate-200 rounded-2xl px-4 py-3.5 text-sm text-slate-700 placeholder-slate-300 focus:outline-none focus:border-[#2ecc71]" />
          </div>

          <div>
            <label className="block text-xs font-bold text-[#1a233d] uppercase tracking-wider mb-2">Senha</label>
            <input type="password" placeholder="Mínimo 8 caracteres" required className="w-full bg-white border border-slate-200 rounded-2xl px-4 py-3.5 text-sm text-slate-700 placeholder-slate-300 focus:outline-none focus:border-[#2ecc71]" />
          </div>
        </div>

        {/* Botão de Envio Inferior Dinâmico */}
        <div className="mt-8 space-y-4">
          <button
            type="submit"
            className={`w-full text-white font-semibold py-4 rounded-2xl transition shadow-lg ${
              role === "freelancer" ? "bg-[#2ecc71] hover:bg-[#27ae60] shadow-emerald-100" : "bg-[#1a233d] hover:bg-[#111827] shadow-slate-200"
            }`}
          >
            Criar conta como {role === "freelancer" ? "Freelancer" : "Empresa"}
          </button>
          
          <p className="text-center text-xs text-slate-400">
            Já tem conta? <Link to="/login" className="text-[#2ecc71] font-bold hover:underline">Entrar</Link>
          </p>
        </div>
      </form>
    </div>
